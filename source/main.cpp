...
void castRayDebug(vec4 p0, vec4 dir){
  ...
  for(unsigned int i=0; i < sceneObjects.size(); i++){
    ...
    //Object::IntersectionValues result=sceneObjects[0]->intersect(p0, E);
    if (intersections[0].t_w!=std::numeric_limits<double>::infinity())
    {
      std::cout << "Hit " << intersections[0].name << " " << intersections[0].ID_ << "\n";
...
/* -------------------------------------------------------------------------- */
float shadowSoftFeeler(vec4 p0, Object *object){
    int inShadow = 0;
    for(unsigned int k=0;k<num;k++)
    {
        vec4 currentlightPosition=point4( ((float)rand()/RAND_MAX)-0.5, 1.5, ((float)rand()/RAND_MAX)-0.5, 1.0 );
        vec4 L=(currentlightPosition-p0);
        float ED=length(L);
        std::vector<Object::IntersectionValues>results(sceneObjects.size());
        for (unsigned int i=0; i<sceneObjects.size();i++)
        {   //std::cout << "hitting:" << (float(inShadow)/num) << "\n";
            results[i]=sceneObjects[i]->intersect(p0, normalize(L));
    ...
    return (1.00-float(inShadow)/num);
...
//color4 material_ambient(sceneObjects[id]->shadingValues.Ka);
//color4 ambient_product=lightColor*material_ambient;
        
       
        vec4 L=normalize(lightPosition-results[0].P_w);
        //vec4 N_w=normalize(results[id].P_w);
        vec4 R=normalize(reflect(L,results[0].N_w));
        vec4 V=normalize(E);
        double kD=fmax(dot(L,results[0].N_w),0.0);
        vec4 kS=pow(fmax(dot(V,R),0.0),sceneObjects[results[0].ID_]->shadingValues.Kn);
        if( dot(L, results[0].N_w) < 0.0 )
        {
        ...
        {
            color = kD * sceneObjects[results[0].ID_]->shadingValues.color + kS * vec4(sceneObjects[results[0].ID_]->shadingValues.Ks, sceneObjects[results[0].ID_]->shadingValues.Ks, sceneObjects[results[0].ID_]->shadingValues.Ks, 1.0);
        
            //color = kS * sceneObjects[results[0].ID_]->shadingValues.color;
            //if(shadowFeeler(results[0].P_w, sceneObjects[results[0].ID_])==true)
            {
                color= color * shadowSoftFeeler(results[0].P_w, sceneObjects[results[0].ID_]);
            ...
            if(sceneObjects[results[0].ID_]->shadingValues.Ks>0)
            {
                ...
                vec4 Rr=normalize(E)-2*normalize(results[0].N_w)*dot(normalize(E),normalize(results[0].N_w));
                color=castRay(results[0].P_w,Rr, sceneObjects[results[0].ID_], depth+1);
            }
        ...
        if(sceneObjects[results[0].ID_]->shadingValues.Kt>0)
        {
            ...
            {
                vec4 refray = n12*E + (n12*c-sqrt(in_sqrt))*results[0].N_w;
                refray=normalize(refray);
                vec4 color2 = castRay(results[0].P_w,refray, sceneObjects[results[0].ID_], depth+1);
                color2= color2 * sceneObjects[results[0].ID_]->shadingValues.Kt;
                color= color+color2;
            }
        }
...
void initCornellBox(){
  ...
  sceneObjects[sceneObjects.size()-1]->setShadingValues(_shadingValues);
  sceneObjects[sceneObjects.size()-1]->setModelView(Translate(1.0, -1.25, 0.5)*Scale(0.375, 0.375, 0.375));
  }
...
void initUnitSphere(){
  ...
  _shadingValues.Kr = 0.0;
  sceneObjects[sceneObjects.size()-1]->setShadingValues(_shadingValues);
  sceneObjects[sceneObjects.size()-1]->setModelView(RotateY(45)*Translate(1, 0, 0));
  }
    {
        sceneObjects.push_back(new Sphere("Diffuse sphere"));
        Object::ShadingValues _shadingValues;
...
void initUnitSquare(){
  ...
  { //Back Wall
    ...
    Object::ShadingValues _shadingValues;
    _shadingValues.color = vec4(1.0,0.0,0.0,0.0);
    _shadingValues.Ka = 0.0;
    _shadingValues.Kd = 1.0;
    _shadingValues.Ks = 0.0;
    _shadingValues.Kn = 16.0;
